...
- **`WinEventLog` inputs kept landing in the wrong index (`main`/`wineventlog`) despite a correctly-defined app-level `inputs.conf`** — root cause was `etc/system/local/inputs.conf`, a leftover from before the app-based config was introduced, silently winning Splunk's config-file precedence over the new `lab_windows` app on all three forwarders. Diagnosed with `btool inputs list <stanza> --debug`, which surfaces the actual winning file per key rather than guessing from contents. Full writeup: [splunk_index_precedence_EN.md](docs/splunk_index_precedence_EN.md)
